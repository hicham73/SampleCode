import { Resolver, Query, ResolveField, Args, Int, Mutation, Parent, InputType, Field } from '@nestjs/graphql';
...
export class TransporteurResolver {
  ...
  async propositions(@Parent() transporteur: Transporteur) {
    return await Proposition.createQueryBuilder().where(`transporteurId = '${transporteur.id}' `).getMany(); 
  }
